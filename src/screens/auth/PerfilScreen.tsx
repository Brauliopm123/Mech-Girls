...
export default function PerfilScreen({ navigation }: any) {
  ...
  // Reconocimientos: PDFs generados desde la web (Supabase Storage).
  // Aquí solo contamos cuántos hay; el listado vive en ConstanciasScreen.
  const cargarReconocimientos = useCallback(async () => {
    if (!usuario?.id_usuario) return;
    const { data, error } = await supabase
      .from('reconocimientos')
      .select('id_reconocimiento')
      .eq('id_usuario', usuario.id_usuario);
  ...
  useEffect(() => { cargar(); cargarReconocimientos(); }, [cargar, cargarReconocimientos]);
      ...
      {reconocimientos.length > 0 && (
        <View style={styles.constanciasSection}>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.constanciaTitulo}>Mis reconocimientos</Text>
              <Text style={styles.constanciaFecha}>
                {reconocimientos.length} disponible{reconocimientos.length !== 1 ? 's' : ''}
              </Text>
            </View>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); cargar(); cargarReconocimientos(); }} tintColor="#E91E63" />
          }
          ListEmptyComponent={
